...
class SpeedMDataField : public MDataField
{
 public:
  ...
  virtual void toFile(std::ofstream& os) const;
...
class CffexMDataField : public MDataField
{
 public:
  virtual const std::string& instruID() const
  {
...
std::basic_ostream< CharT, TraitsT >& operator<<(std::basic_ostream< CharT, TraitsT >& os, CffexMDataField const& aDepthMarketData)
{
    os <<std::endl;
    os <<"{" <<std::endl;
...
class MDataFieldCallback
{
 public:
  MDataFieldCallback(MData* mdata);

  ~MDataFieldCallback();

  void msgCallback(const MDataField* msg);
...
class MData
{
 public:
  ...
  static MData* create(MDataType type,
  ...
  virtual void pushMData(const MDataField* data) = 0;

  void outMData(const MDataField* data);

 protected:
  std::auto_ptr<soil::MsgQueue<MDataField, MDataFieldCallback> > mdata_queue_;
 ...
 private:
  ...
  std::auto_ptr<MDataFieldCallback> callback_;
};
